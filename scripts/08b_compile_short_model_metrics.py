#### Master Script 7b: Compile performance metrics and calculate confidence intervals ####
#
...
metric_info_df = pd.DataFrame({'file':metric_files,
                               'CUTOFF':[re.search(VERSION+'/(.*)/', curr_file).group(1) for curr_file in metric_files],
                               'ADM_OR_DISCH':[re.search('Day(.*)/tune', curr_file).group(1) for curr_file in metric_files],
                              ...
                              }).sort_values(by=['CUTOFF','ADM_OR_DISCH','TUNE_IDX','RESAMPLE_IDX','METRIC']).reset_index(drop=True)
metric_info_df['METRIC'] = metric_info_df['METRIC'].str.rsplit(pat='/', n=1).apply(lambda x: x[1])
metric_info_df['ADM_OR_DISCH'] = metric_info_df['ADM_OR_DISCH'].str.rsplit(pat='/', n=1).apply(lambda x: x[1])
metric_info_df['CUTOFF'] = metric_info_df['CUTOFF'].str.split(pat='/', n=1).apply(lambda x: x[0])
...
for curr_metric in metric_info_df.METRIC.unique():
    ...
    for cutoff in ['Day2','Day3']:
        
        # Filter files of current metric
        curr_metric_info_df = metric_info_df[(metric_info_df.METRIC==curr_metric)&(metric_info_df.CUTOFF==cutoff)].reset_index(drop=True)
        ...
        if curr_metric == 'ROCs':
            ...
            CI_overall.to_csv(os.path.join(performance_dir,cutoff,'CI_overall_metrics.csv'),index=False)
            ...
            CI_threshold.to_csv(os.path.join(performance_dir,cutoff,'CI_threshold_metrics.csv'),index=False)