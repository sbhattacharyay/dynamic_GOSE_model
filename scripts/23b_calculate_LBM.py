#### Master Script 23b: Calculate LBM for dynAPM_DeepMN ####
#
...
lbm_dir = os.path.join(interp_dir,'LBM')
...
def main(array_task_id):
    ...
    curr_lbm_dir = os.path.join(curr_lbm_fold_dir,'tune'+str(curr_tune_idx).zfill(4),'GUPI_'+curr_gupi)
    ...
    # Iterate through class indices and continuous approximation masks (LBM - Step 1)
    for curr_class_idx in class_indices:
        
        # Define and create sub-sub-directory for the class
        curr_class_dir = os.path.join(curr_lbm_dir,'GOSE_'+gose_classes[curr_class_idx])
        os.makedirs(curr_class_dir,exist_ok=True)
        
        # Initialize Step 1 LBM calculation PyTorch Lightning class
        curr_step1_LBM = LBM_step1_model(curr_class_idx,gose_model,cols_to_add,unknown_index,5,multihot_matrix.shape[0],.005,.5,.1)
        
        # Initialize callback and logging objects
        lr_monitor = LearningRateMonitor(logging_interval='step')
        early_stop_callback = EarlyStopping(monitor='lr_reductions',stopping_threshold=3,patience=5000,mode='max')
#         checkpoint_callback = ModelCheckpoint(monitor='train_loss',dirpath=curr_class_dir,filename='LBM_step1_{train_loss:.2f}',save_top_k=1,mode='min')
#         checkpoint_callback
        csv_logger = pl.loggers.CSVLogger(save_dir=curr_lbm_dir,name='GOSE_'+gose_classes[curr_class_idx])
        ...
        with torch.no_grad():
            ...
            for step2_iter in range(max_iter):
                for timestep in tqdm(range(multihot_matrix.shape[0])):
                    ...
                    for u in U_values:
                        ...
                        curr_output_vector = F.softmax(curr_gose_out)[:,curr_class_idx]
            ...
            LBM_attribution_df['GOSE'] = gose_classes[curr_class_idx]
            ...
            LBM_attribution_df.to_pickle(os.path.join(curr_class_dir,'LBM_dataframe.pkl'))
            
if __name__ == '__main__':
    
    array_task_id = int(sys.argv[1]) + 10000
    main(array_task_id)