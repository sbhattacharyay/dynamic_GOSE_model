#### Master Script 05c (ii): Second pass of calculating TimeSHAP for dynAPM_DeepMN models in parallel ####
#
...
# Create a subdirectory for the storage of second-pass TimeSHAP values
sub_shap_dir = os.path.join(shap_dir,'second_pass_parallel_results')
os.makedirs(sub_shap_dir,exist_ok=True)

# Create a subdirectory for the storage of second-pass missed TimeSHAP transitions
missed_transition_dir = os.path.join(shap_dir,'second_pass_missed_transitions')
...
timeshap_partitions = pd.read_pickle(os.path.join(shap_dir,'second_pass_timeSHAP_partitions.pkl'))
...
def main(array_task_id):
    ...
    curr_transitions = timeshap_partitions[timeshap_partitions.PARTITION_IDX==array_task_id].reset_index(drop=True)
    ...
    for curr_fold in tqdm(curr_transitions.FOLD.unique(),'Iterating through unique folds to calculate TimeSHAP'):
        ...
        for curr_tune_idx in tqdm(curr_transitions[curr_transitions.FOLD==curr_fold].TUNE_IDX.unique(),'Iterating through unique tuning indices in fold '+str(curr_fold)+' to calculate TimeSHAP'):
            ...
            # For second-pass, define average-token dataframe from training set for "average event"
