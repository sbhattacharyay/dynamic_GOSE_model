#### Master Script 22d: Compile performance metrics and calculate confidence intervals ####
#
...
metric_files = []
for path in Path(os.path.join(performance_dir)).rglob('*_from_disch.csv'):
...
metric_info_df = pd.DataFrame({'file':metric_files,
                               ...
                               'METRIC':[re.search('/performance_(.*)_set_', curr_file).group(1) for curr_file in metric_files],
                               'SET':[re.search('_set_(.*)_from_disch.csv', curr_file).group(1) for curr_file in metric_files]
...
for curr_metric in metric_info_df.METRIC.unique():
    ...
    # Calculate 95% confidence intervals
    if curr_metric == 'ROCs':
        CI_compiled_ROCs = compiled_curr_metric_values.groupby(['TUNE_IDX','WINDOW_IDX','THRESHOLD','FPR'],as_index=False)['TPR'].aggregate({'TPR_mean':np.mean,'TPR_std':np.std,'TPR_median':np.median,'TPR_lo':lambda x: np.quantile(x,.025),'TPR_hi':lambda x: np.quantile(x,.975),'resamples':'count'}).reset_index(drop=True)
        ...
        CI_compiled_ROCs.to_csv(os.path.join(performance_dir,'CI_ROCs_from_disch.csv'))
        
    elif curr_metric == 'calibration_curves':
        CI_compiled_calibration = compiled_curr_metric_values.groupby(['TUNE_IDX','WINDOW_IDX','THRESHOLD','PREDPROB'],as_index=False)['TRUEPROB'].aggregate({'TRUEPROB_mean':np.mean,'TRUEPROB_std':np.std,'TRUEPROB_median':np.median,'TRUEPROB_lo':lambda x: np.quantile(x,.025),'TRUEPROB_hi':lambda x: np.quantile(x,.975),'resamples':'count'}).reset_index(drop=True)
        ...
        CI_compiled_calibration.to_csv(os.path.join(performance_dir,'CI_calibration_from_disch.csv'))
        ...
        CI_overall.to_csv(os.path.join(performance_dir,curr_set+'_CI_overall_metrics_from_disch.csv'),index=False)
        ...
        CI_threshold.to_csv(os.path.join(performance_dir,curr_set+'_CI_threshold_metrics_from_disch.csv'),index=False)