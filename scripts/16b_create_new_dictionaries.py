#### Master Script 16b: Train dictionaries and convert tokens to embedding layer indices ####
#
...
indexing_combos = uniq_partitions.merge(adm_disch_df,how='outer').merge(strategy_df,how='outer').reset_index(drop=True)
indexing_combos = indexing_combos[indexing_combos.strategy == 'abs'].reset_index(drop=True)
...
def main(array_task_id):
    ...
    train_tokens = pd.concat([pd.read_csv(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_GUPI_'+g+'_strategy_'+curr_strategy+'.csv')) for g in tqdm(train_GUPIs,desc='Collecting '+curr_adm_or_disch+' training tokens')],ignore_index=True).fillna('')
    
    # Load and compile testing set tokens
    test_tokens = pd.concat([pd.read_csv(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_GUPI_'+g+'_strategy_'+curr_strategy+'.csv')) for g in tqdm(test_GUPIs,desc='Collecting '+curr_adm_or_disch+' testing tokens')],ignore_index=True).fillna('')
    
    # Delete all used token files once index files are stored to save disk space
    token_file_list = [(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_GUPI_'+g+'_strategy_'+curr_strategy+'.csv')) for g in cv_splits.GUPI.unique()]
    token_file_list = [f for f in token_file_list if os.path.exists(f)]
    [os.remove(f) for f in tqdm(token_file_list,desc='Deleting current fold '+curr_adm_or_disch+' token files for strategy: '+curr_strategy)]
    ...
    cp.dump(curr_vocab, open(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_strategy_'+curr_strategy+'_token_dictionary.pkl'), "wb" ))
    
    # Convert training set tokens to indices
    train_tokens['VocabIndex'] = [curr_vocab.lookup_indices(train_tokens.Token[curr_row].split(' ')) for curr_row in tqdm(range(train_tokens.shape[0]),desc='Converting '+curr_adm_or_disch+' training tokens to indices for strategy: '+curr_strategy)]
    train_tokens = train_tokens.drop(columns='Token')
    
    # Convert testing set tokens to indices
    test_tokens['VocabIndex'] = [curr_vocab.lookup_indices(test_tokens.Token[curr_row].split(' ')) for curr_row in tqdm(range(test_tokens.shape[0]),desc='Converting '+curr_adm_or_disch+' testing tokens to indices for strategy: '+curr_strategy)]
    test_tokens = test_tokens.drop(columns='Token')
    
    # Store training and testing indices into selected directory
    train_tokens.to_pickle(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_strategy_'+curr_strategy+'_training_indices.pkl'))
    test_tokens.to_pickle(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_strategy_'+curr_strategy+'_testing_indices.pkl'))
