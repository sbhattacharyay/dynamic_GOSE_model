#### Master Script 4b: Compile predictions of dynamic all-predictor-based models ####
#
...
# Custom methods
from functions.model_building import load_predictions
...
pred_file_info_df = pd.DataFrame({'file':pred_files,
                                  'adm_or_disch':[re.search('fold(.*)/', curr_file).group(1) for curr_file in pred_files],
                                 ...
                                 }).sort_values(by=['repeat','fold','adm_or_disch','VERSION']).reset_index(drop=True)
...
adm_val_preds = load_predictions(adm_val_info_df, progress_bar_desc='Compiling validation set predictions from admission')
adm_test_preds = load_predictions(adm_test_info_df, progress_bar_desc='Compiling testing set predictions from admission')

disch_val_preds = load_predictions(disch_val_info_df, progress_bar_desc='Compiling validation set predictions from discharge')
disch_test_preds = load_predictions(disch_test_info_df, progress_bar_desc='Compiling testing set predictions from discharge')
