#### Master Script 11b: Compile performance metrics and calculate confidence intervals ####
#
...
metric_info_df = pd.DataFrame({'file':metric_files,
                               'ADM_OR_DISCH':[re.search(VERSION+'/(.*)/tune', curr_file).group(1) for curr_file in metric_files],
                              ...
                              }).sort_values(by=['ADM_OR_DISCH','TUNE_IDX','RESAMPLE_IDX','METRIC']).reset_index(drop=True)
