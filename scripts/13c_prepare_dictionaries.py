#### Master Script 13c: Train dictionaries and convert tokens to embedding layer indices ####
#
...
def main(array_task_id):
    ...
    train_tokens = pd.concat([pd.read_csv(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_'+g+'.csv')) for g in tqdm(train_GUPIs,desc='Collecting '+curr_adm_or_disch+' training tokens')],ignore_index=True).fillna('')
    
    # Load and compile testing set tokens
    test_tokens = pd.concat([pd.read_csv(os.path.join(fold_dir,'from_'+curr_adm_or_disch+'_'+g+'.csv')) for g in tqdm(test_GUPIs,desc='Collecting '+curr_adm_or_disch+' testing tokens')],ignore_index=True).fillna('')
    ...
    train_tokens.VocabIndex[train_tokens.Token == ''] = np.nan
    ...
    test_tokens.VocabIndex[test_tokens.Token == ''] = np.nan
