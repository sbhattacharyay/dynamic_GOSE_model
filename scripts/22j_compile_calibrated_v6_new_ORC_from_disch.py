#### Master Script 22j: Compile new ORC from discharge and calculate confidence intervals ####
#
...
metric_files = []
for path in Path(os.path.join(performance_dir)).rglob('*_new_orc_from_disch.csv'):
...
metric_info_df = pd.DataFrame({'file':metric_files,
                               ...
                               'METRIC':'NEW_ORC',
                               'SET':[re.search('_set_(.*)_new_orc', curr_file).group(1) for curr_file in metric_files]
...
CI_overall.to_csv(os.path.join(performance_dir,curr_set+'_CI_new_ORC_from_disch.csv'),index=False)