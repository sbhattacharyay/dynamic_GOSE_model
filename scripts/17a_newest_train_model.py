#### Master Script 17a: Retrain dynamic all-predictor-based models to combat overfitting ####
#
...
if not os.path.exists(os.path.join(model_dir,'tuning_grid.csv')):
    ...
    # Fix the minibatch size
    tuning_grid.BATCH_SIZE = 1
    
    # Drop the tuning index column
    ...
    uniq_window_limit = pd.DataFrame({'WINDOW_LIMIT':[12,24,36,84,'None'],'key':1})
    uniq_strategies = pd.DataFrame({'STRATEGY':['abs','diff'],'key':1})
    tuning_combos_df = pd.merge(uniq_window_limit,uniq_strategies,how='outer',on='key')
    ...
    tuning_grid = tuning_grid[['tune_idx','WINDOW_LIMIT','STRATEGY','BATCH_SIZE','RNN_TYPE','LATENT_DIM','EMBED_DROPOUT','HIDDEN_DIM','RNN_LAYERS','NUM_EPOCHS','ES_PATIENCE','IMBALANCE_CORRECTION','OUTPUT_ACTIVATION','LEARNING_RATE','repeat','fold']].reset_index(drop=True)
...
tuning_grid = tuning_grid[(tuning_grid.tune_idx == 9)&(tuning_grid.fold.isin([1,3]))].reset_index(drop=True)
