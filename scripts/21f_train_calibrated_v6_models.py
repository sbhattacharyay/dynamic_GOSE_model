...
tuning_grid = tuning_grid[tuning_grid.TUNE_IDX.isin([55,135])].drop(columns=['repeat','fold']).drop_duplicates().reset_index(drop=True)
tuning_grid['key'] = 1
partitions['key'] = 1
...
def main(array_task_id):
    ...
    # Load best calibration configurations for current tuning index
    calib_configs = pd.read_pickle(os.path.join(calibration_dir,'best_configurations_tune_'+str(curr_tune_idx).zfill(4)+'.pkl'))
    ...
    calibrated_val_preds.append(curr_val_preds[curr_val_preds.WindowIdx >= 85].reset_index(drop=True))
    calibrated_test_preds.append(curr_test_preds[curr_test_preds.WindowIdx >= 85].reset_index(drop=True))
    
    # Learn calibration parameters from validation set predictions
    for curr_wi in range(1,85):
        ...
        curr_optimization = calib_configs.OPTIMIZATION[(calib_configs.WINDOW_IDX == curr_wi).idxmax()]
        curr_calibration = calib_configs.CALIBRATION[(calib_configs.WINDOW_IDX == curr_wi).idxmax()]
        
        if curr_calibration == 'None':
            
            calibrated_val_preds.append(curr_wi_val_preds)
            calibrated_test_preds.append(curr_wi_test_preds)
            
        elif curr_calibration == 'T':
            ...
            scale_object = TemperatureScaling(curr_wi_val_preds)
            ...
            calib_val_logits = torch.tensor((curr_wi_val_preds[logit_cols] / opt_temperature).values,dtype=torch.float32)
            ...
            calib_test_logits = torch.tensor((curr_wi_test_preds[logit_cols] / opt_temperature).values,dtype=torch.float32)
