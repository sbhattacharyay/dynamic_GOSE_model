#### Master Script 5a: Assess model performance ####
#
...
window_indices = [i+1 for i in range(WINDOW_LIMIT)]
...
# Expand resampling information by 'adm/disch' and 'WindowIdx'
uniq_models = pd.DataFrame({'adm_or_disch':['adm','disch'],'key':1})
bs_resamples['key'] = 1
...
def main(array_task_id):
    ...
    metric_dir = os.path.join(perf_dir,curr_adm_or_disch,'resample'+str(curr_rs_idx).zfill(4))
    os.makedirs(metric_dir,exist_ok=True)
    
    # Load compiled testing set predictions
    compiled_test_preds = pd.read_csv(os.path.join(model_dir,'compiled_test_predictions_from_'+curr_adm_or_disch+'.csv'),index_col=0)
    ...
    for curr_wi in window_indices:
        ...
        orcs.append(pd.DataFrame({'ADM_OR_DISCH':curr_adm_or_disch,'RESAMPLE_IDX':curr_rs_idx,'WINDOW_IDX':curr_wi,'METRIC':'ORC','VALUE':curr_orc},index=[0]))
    ...
    for curr_wi in window_indices:
        ...
        somers_d.append(pd.DataFrame({'ADM_OR_DISCH':curr_adm_or_disch,'RESAMPLE_IDX':curr_rs_idx,'WINDOW_IDX':curr_wi,'METRIC':'Somers D','VALUE':curr_somers_d},index=[0]))
    ...
    overall_metrics.to_csv(os.path.join(metric_dir,'overall_metrics.csv'),index=False)
