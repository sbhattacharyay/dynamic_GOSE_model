#### Master Script 5b: Compile performance metrics and calculate confidence intervals ####
#
...
adm_metric_files = []
for path in Path(os.path.join(performance_dir,'adm')).rglob('*.csv'):
    adm_metric_files.append(str(path.resolve()))
    
# Search for all performance metric files in the directory
disch_metric_files = []
for path in Path(os.path.join(performance_dir,'disch')).rglob('*.csv'):
    disch_metric_files.append(str(path.resolve()))

# Concatenate lists of performance metric files
metric_files = adm_metric_files+disch_metric_files

# Characterise list of discovered performance metric files
metric_info_df = pd.DataFrame({'file':metric_files,
                               'ADM_OR_DISCH':[re.search('model_performance/(.*)/resample', curr_file).group(1) for curr_file in metric_files],
                              ...
                              }).sort_values(by=['ADM_OR_DISCH','RESAMPLE_IDX','METRIC']).reset_index(drop=True)
...
for curr_metric in metric_info_df.METRIC.unique():
    ...
    if curr_metric == 'ROCs':
        CI_compiled_ROCs = compiled_curr_metric_values.groupby(['ADM_OR_DISCH','WINDOW_IDX','THRESHOLD','FPR'],as_index=False)['TPR'].aggregate({'TPR_mean':np.mean,'TPR_std':np.std,'TPR_median':np.median,'TPR_lo':lambda x: np.quantile(x,.025),'TPR_hi':lambda x: np.quantile(x,.975),'resamples':'count'}).reset_index(drop=True)
    ...
    elif curr_metric == 'overall_metrics':
        CI_overall = compiled_curr_metric_values.groupby(['ADM_OR_DISCH','WINDOW_IDX','METRIC'],as_index=False)['VALUE'].aggregate({'mean':np.mean,'std':np.std,'median':np.median,'lo':lambda x: np.quantile(x,.025),'hi':lambda x: np.quantile(x,.975),'resamples':'count'}).reset_index(drop=True)
        CI_overall.to_csv(os.path.join(performance_dir,'CI_overall_metrics.csv'),index=False)

    elif curr_metric == 'threshold_metrics':
        macro_compiled_threshold = compiled_curr_metric_values.groupby(['ADM_OR_DISCH','WINDOW_IDX','RESAMPLE_IDX','METRIC'],as_index=False)['VALUE'].mean()
        ...
        CI_threshold = compiled_curr_metric_values.groupby(['ADM_OR_DISCH','WINDOW_IDX','THRESHOLD','METRIC'],as_index=False)['VALUE'].aggregate({'mean':np.mean,'std':np.std,'median':np.median,'lo':lambda x: np.quantile(x,.025),'hi':lambda x: np.quantile(x,.975),'resamples':'count'}).reset_index(drop=True)
